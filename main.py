...
def update_schedule_form(request: Request, schedule_id: int, debt_id: int = Form(...), group: str = Form(...), teacher: str = Form(...), date: datetime = Form(...), db: Session = Depends(get_db)):
    schedule = schemas.ScheduleCreate(debt_id=debt_id, group=group, teacher=teacher, date=date)
...
def update_schedule_page(request: Request, schedule_id: int, debt_id: int = Form(...), group: str = Form(...), teacher: str = Form(...), date: datetime = Form(...), db: Session = Depends(get_db)):
    schedule = schemas.ScheduleCreate(debt_id=debt_id, group=group, teacher=teacher, date=date)
    crud.update_schedule(db, schedule_id, schedule)
    return RedirectResponse(url=f"/schedule/{schedule_id}", status_code=303)
